...
fn sign_message(
    ...
    let signature = tapp_cli::app_key::sign_message(&private_key, message_bytes)?;
...
fn verify_signature(
    ...
    let is_valid = tapp_cli::app_key::verify_signature(&public_key, message_bytes, &signature)?;
...
async fn node_override_hashes(
    ...
    let (app_compose, app_volumes) =
        tapp_cli::onchain::get_app_default_hashes(rpc_url, contract, app_id).await?;
...
async fn register_onchain(
    ...
    use tapp_cli::onchain::OnchainParams;
    ...
    let tx = tapp_cli::onchain::register_app(
        &params,
        &app_id,
        compose_hash,
        volumes_hash,
...
async fn update_onchain(
    ...
    let params = OnchainParams { rpc_url, contract, private_key };
    let tx = tapp_cli::onchain::update_app(&params, &app_id, compose_hash, volumes_hash, image_hashes).await?;
...
async fn add_node_onchain(
    ...
    let params = OnchainParams { rpc_url, contract, private_key };
    let tx = tapp_cli::onchain::add_node(
...
async fn remove_node_onchain(
    ...
    let params = OnchainParams { rpc_url, contract, private_key };
    let tx = tapp_cli::onchain::remove_node(&params, &app_id, signer).await?;
...
async fn update_node_onchain(
    ...
    let params = OnchainParams { rpc_url, contract, private_key };
    let tx = tapp_cli::onchain::update_node(
...
async fn withdraw_onchain(
    ...
    use tapp_cli::onchain::{self, OnchainParams};

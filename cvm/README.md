...
- **Stage A** (`build-tapp.sh`): provision app / docker / SGX / DNS, security hardening, Sysbox, and the `/data` + `br_netfilter` bakes. Also bakes **tapp-front**: an nginx TLS front on **:50052** proxying the plaintext gRPC :50051, with a self-signed cert regenerated in `/run` on every boot — remote `tapp-cli --server https://<host>:50052 --insecure` is encrypted from first boot (claim included). Encryption only: the front carries no identity claim and needs no separate attestation, being part of the measured image.

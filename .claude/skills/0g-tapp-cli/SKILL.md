...
- **Server**: `-s <url>` (e.g. `http://<host>:50051`). There are MANY tapp servers; always pass `-s` explicitly.
  ...
  - Honest limits: `--insecure` defeats **passive observers only** — an active on-path attacker can terminate TLS and relay, and (since request signatures cover method+timestamp, not the body) even rewrite request contents under a valid authorization. When that matters, pin the peer with `--tls-pin <SPKI sha256, hex or sha256//base64>` (obtain it out-of-band), or front the node with a CA-issued cert.

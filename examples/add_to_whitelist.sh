...
# Add address to whitelist with Ethereum signature authentication
# Usage:
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --help|-h)
      ...
      echo "  --address ADDRESS    EVM address to add to whitelist (required)"
      ...
      echo ""
      echo "Note: Only the owner can add addresses to whitelist"
...
response=$(printf "%s" "$request_json" | tr -d '\n' | \
  grpcurl -plaintext \
    ...
    -d @ \
    "$TARGET_ADDRESS" \
    tapp_service.TappService/AddToWhitelist 2>&1)
exit_code=$?
set -e
...
if echo "$response" | jq -e '.success == true' > /dev/null 2>&1; then
  echo "✅ Successfully added $ADDRESS_TO_ADD to whitelist"
else
  echo "❌ Failed to add address to whitelist"

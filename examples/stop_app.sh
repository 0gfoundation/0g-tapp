...
# Stop application with Ethereum signature authentication
# Usage:
#   ./stop_app.sh [OPTIONS]
#
# Examples:
#   ./stop_app.sh --host 39.97.63.199 --app-id test-nginx-app --use-owner
#   ./stop_app.sh --host 39.97.63.199 --app-id test-nginx-app --use-whitelist
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo "  --app-id APP_ID         Application ID to stop (default: $DEFAULT_APP_ID)"
        ...
        *)
            ...
            # Original order: APP_ID HOST PORT
            if [ -z "$APP_ID" ]; then
                APP_ID="$1"
...
response=$(printf "%s" "$request_json" | tr -d '\n' | \
    grpcurl -plaintext \
        ...
        -d @ \
        "$TARGET_ADDRESS" \
        tapp_service.TappService/StopApp 2>&1)
exit_code=$?
set -e
